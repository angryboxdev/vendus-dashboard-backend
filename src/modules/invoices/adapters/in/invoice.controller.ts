...
import type { CreateInvoicePort, UpdateInvoicePort, MarkInvoicePaidPort, SetInvoiceStatusPort, ClassifyInvoiceLinePort, ListInvoicesPort, GetInvoicePort, DeleteInvoicePort, SuggestLineClassificationPort } from "../../domain/ports/in/invoice.ports.js";
  ...
  router.patch("/invoices/:invoiceId/lines/:lineId/classify", async (req, res) => {
    try {
      const { classify, saveAsRule } = req.body as {
        classify: { type?: InvoiceLineType; costCenterId?: string | null; category?: string | null; subcategory?: string | null };
        saveAsRule?: boolean;
      };
