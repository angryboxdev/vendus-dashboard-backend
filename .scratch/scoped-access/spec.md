...
| Spec C: per-organization credentials, cron fan-out | Same reasoning as the row above, for the crons specifically — they still use `UNATTENDED_SCOPE` unchanged, unaffected by `location-credentials` ticket 06 (ADR-0009's amendment covers kiosk/till-closing/KDS only). **It deletes `UNATTENDED_SCOPE`**, which is the trigger recorded here | immediately after `location-credentials` (spec E) |
