...
describe("CreateCostCenterGroupUseCase", () => {
  ...
  it("cria e persiste um grupo", async () => {
    ...
    const result = await useCase.execute({ code: "OPD", name: "Operação Direta", sortOrder: 1 });
  ...
  it("normaliza o código para maiúsculas", async () => {
    ...
    const result = await useCase.execute({ code: "pes", name: "Pessoal" });
    expect(result.code).toBe("PES");
  });

  it("lança erro se o código já existe", async () => {
    const { useCase } = makeUseCase();
    await useCase.execute({ code: "OPD", name: "Operação Direta" });

    await expect(useCase.execute({ code: "OPD", name: "Outra" })).rejects.toThrow(
