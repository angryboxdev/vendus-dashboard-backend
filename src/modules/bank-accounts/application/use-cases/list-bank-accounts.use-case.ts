...
import type { ListBankAccountsPort, BankAccountDto } from "../../domain/ports/in/bank-accounts.ports.js";
...
export class ListBankAccountsUseCase implements ListBankAccountsPort {
  ...
  async execute(bankId: string): Promise<BankAccountDto[]> {
    const accounts = await this.repo.findByBankId(bankId);
