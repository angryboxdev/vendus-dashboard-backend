...
describe("MarkOccurrenceAsPaidUseCase", () => {
  ...
  it("marca ocorrência invoice_linked como paid quando requireInvoice=true", async () => {
    ...
    await link.execute({ organizationId, occurrenceId: occ.id, invoiceId: "inv-1" });

    const result = await markPaid.execute({ organizationId, occurrenceId: occ.id });
  ...
  it("lança OccurrenceInvoiceRequiredError se requireInvoice=true e fatura não vinculada", async () => {
    ...
    await expect(markPaid.execute({ organizationId, occurrenceId: occ.id })).rejects.toThrow(
      OccurrenceInvoiceRequiredError,
    );
  });
  ...
  it("lança OccurrenceInvalidTransitionError se já está paid", async () => {
    ...
    const rec = await create.execute(FIXED_CMD);
    const occ = await generate.execute({ organizationId, recurrenceId: rec.id, year: 2026, month: 9 });
    await markPaid.execute({ organizationId, occurrenceId: occ.id });
  ...
  it("lança OccurrenceNotFoundError para id inexistente", async () => {
    const { markPaid } = make();
    await expect(markPaid.execute({ organizationId, occurrenceId: "nao-existe" })).rejects.toThrow(
      OccurrenceNotFoundError,
    );
  });
  ...
  it("lança OccurrenceNotFoundError se a ocorrência pertence a outra organização", async () => {
    ...
    await expect(
      markPaid.execute({ organizationId: otherOrganizationId, occurrenceId: occ.id }),
