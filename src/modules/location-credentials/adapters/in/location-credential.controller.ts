...
export class LocationCredentialController {
  ...
  private registerDeviceRoutes(): void {
     ...
     * rejects with 401 for a missing, unknown or revoked token alike
     * (README "Tokens are deleted, not marked revoked"; the
     * `UNATTENDED_SCOPE` fallback for a wholly absent token was removed in
     * ticket 06) — so this is a thin pass-through that echoes back the
     * scope the middleware resolved, rather than a second read of something
     * already proven. No new use-case/port: there is no domain decision
     * left to make here.
     */
