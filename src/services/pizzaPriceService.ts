...
export async function listPizzaPrices(organizationId: OrganizationId, pizzaId: string): Promise<PizzaPrice[]> {
  ...
  return ((data ?? []) as unknown as Row[]).map(rowToPrice);
}

export async function getPizzaPrice(organizationId: OrganizationId, id: string): Promise<PizzaPrice | null> {
  const { data, error } = await createScopedQuery(organizationId)
    .table("pizza_prices")
    .select("id, pizza_id, size, price, created_at, updated_at")
    .eq("id", id)
  ...
  return rowToPrice(data as unknown as Row);
}

export async function createPizzaPrice(
  organizationId: OrganizationId,
  body: PizzaPriceCreateBody
): Promise<PizzaPrice> {
...
export async function deletePizzaPrice(organizationId: OrganizationId, id: string): Promise<void> {
  const { error } = await createScopedQuery(organizationId).table("pizza_prices").delete().eq("id", id);
