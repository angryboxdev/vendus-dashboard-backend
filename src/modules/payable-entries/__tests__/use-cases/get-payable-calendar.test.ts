...
describe("GetPayableCalendarUseCase", () => {
  ...
  it("groups entries by day within range", async () => {
    ...
    await create.execute({ organizationId, supplierName: "A", description: "D", amount: 5000, dueDate: "2026-07-10" });
    await create.execute({ organizationId, supplierName: "B", description: "D", amount: 3000, dueDate: "2026-07-10" });
    await create.execute({ organizationId, supplierName: "C", description: "D", amount: 8000, dueDate: "2026-07-20" });
    await create.execute({ organizationId, supplierName: "D", description: "D", amount: 9000, dueDate: "2026-08-01" }); // outside range

    const uc = new GetPayableCalendarUseCase(repo);
    const days = await uc.execute({ organizationId, from: "2026-07-01", to: "2026-07-31" });
