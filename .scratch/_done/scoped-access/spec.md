...
| ~~Location credentials for kiosk, till-closing, KDS~~ — **closed by `location-credentials` ticket 06** | With one organization every screen already belonged to it. **Resolved**: ticket 06 made the Location token mandatory and removed the `UNATTENDED_SCOPE` fallback for these three consumers, replacing D6's shared constant with a real per-Location, revocable credential — see `docs/adr/0010` and ADR-0009's amendment. Renamed from the prior register's "device identity," which this row and the next one split by consumer (`.scratch/location-credentials/spec.md` D1) | closed |
| ~~Kiosk PIN collision across organizations~~ — **closed by `location-credentials` ticket 02** | A four-digit PIN is a 10,000-value space; with two organizations a collision lets an employee verify against another tenant's employee. Contained while one organization exists. Carries a front-end contract change. **Resolved**: once the kiosk route resolves its organization from `requireDeviceAuth` (a real paired token) instead of the hardcoded `UNATTENDED_SCOPE`, `findActiveEmployeeByPinHash`'s existing `createScopedQuery(organizationId)` call already filters by that organization before the PIN-hash predicate runs — no query change was needed, only wiring the real scope through. See `.scratch/location-credentials/issues/02-convert-kiosk-to-device-auth.md` Comments. | closed |
...
| ~~Spec C: per-organization credentials, cron fan-out~~ — **closed by `org-integration-credentials` ticket 08** | Same reasoning as the row above, for the crons specifically — they used to both use `UNATTENDED_SCOPE` unchanged, unaffected by `location-credentials` ticket 06 (ADR-0009's amendment covers kiosk/till-closing/KDS only). **Resolved, partially**: ticket 06 converted `process-direct-debits` to fan out over every organization; it no longer reads `UNATTENDED_SCOPE`. `daily-vendus-consumption`'s own conversion (ticket 05) is **won't-do** — that cron is disabled with no plan to re-enable it — so it, `server.ts`'s boot-time credential resolution, the manual stock-adjustment/reset scripts, and this spec's own cutover scripts remain genuine, named consumers of `UNATTENDED_SCOPE`; it is not deleted. See `docs/adr/0014`, `src/infra/scoped-db/unattended-scope.ts`'s header, and `.scratch/org-integration-credentials/issues/08-closing-adr-and-corrections.md` | closed |
